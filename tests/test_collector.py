...
class FakeGateway:
    ...
    async def bootstrap_cursor(self, _session, peer, _now):
        if peer != PEER:
...
class CollectorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_positive_private_dialog_is_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            seed_locked(paths)
            # Recreate an authenticated setup state without the lock.
            paths.chat_locked.unlink()
            settings = read_json(paths.settings)
            settings.update(phase="authenticated", chat_locked=False)
            for key in ("source_id", "chat_id", "chat_title", "peer", "initial_message_id",
    ...
    async def test_reset_overtakes_hung_setup_network_operation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            settings.update(phase="authenticated", chat_locked=False)
            for key in (
                    "source_id", "chat_id", "chat_title", "peer",
                    "initial_message_id", "upload_public_key",
